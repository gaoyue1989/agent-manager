...
| **租约丢失期间的 seq 冲突（残余风险，建议加固）** | `seedSeq` 从 DB 最大 seq 续起并 `incrementAndGet()`，其安全性完全依赖「同一 session 任一时刻只有一个 Pod 在写」这一前提（Task 1 的 javadoc 已声明）。**该前提有一个未被覆盖的破口**：`TurnLeaseGuard` 丢失租约时只停止续租（`renewer.shutdownNow()` + 释放），**不会中止 agent 执行**。因此若 Pod A 的续租连续失败至 TTL 过期（TTL 60s / 续租间隔 20s，即约 3 次连续失败，或一次长 GC 停顿）而执行线程仍在产出事件，Pod B 接管后两侧会从重叠的 seq 区间分配。后果**是静默的**：`session_event` 上只有非唯一索引 `KEY idx_session_seq (session_id, seq)`（`durable-sse-plan.md:107` 的原始设计亦如此，属设计缺口而非实现遗漏），所以冲突不会报错，而是写入重复 seq 行——重连客户端按游标回放会看到重复事件（不会挂起，因为 `cursor = e.seq()` 仍在推进）。实时 SSE 不受影响（走广播，不读 DB）。**加固建议（未做，需改 DDL 与迁移，故留决策）**：把 `idx_session_seq` 改为 `UNIQUE KEY`，让冲突变成一次响亮的 `insertBatch` 失败（`append` 返回 -1，广播仍正常），从而把静默损坏转为可观测失败。注意 `CREATE TABLE IF NOT EXISTS` 对既有库不生效，需配一条幂等 `ALTER`；执行前先查既有数据是否存在重复 `(session_id, seq)`，否则加索引会失败。 |
