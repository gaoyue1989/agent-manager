...
## 11. 实施记录与发现（2026-09-19，M0-M6 落地）

### 11.1 实施结果

| 组 | 用例数 | 结果 |
|----|--------|------|
| api-core（S/F/H/M/A） | 27 | ✅ 全绿 |
| api-sandbox（X） | 6 + 2 fixme | ✅ 全绿 |
| ui（U） | 9 + 1 fixme | ✅ 全绿 |
| api-multi + ui-multi（R/U9） | 6 + 1 | 本地多轮迭代后收敛，CI 实测确认 |

### 11.2 录制回放架构的关键实现语义

1. **调用索引按请求形状推导**：请求以 user 结尾 → calls[0]；以 tool 结尾 → tool 消息条数即索引；拒绝恢复（末条 tool 含 "Permission denied"）→ variants.denied。
2. **录制期策展**（record-llm flush）：过滤后台记忆提取调用（system 含 "memory extraction assistant"）+ 前导裁剪（首个真实 tool_call/非空 content 之前的噪声调用，如 mimo 的 NO_REPLY）+ 剥除模型顽固附传的 `, "content": null` + `{{appId}}` 占位符改写。
...
6. **双流 id 语义不同**：chat 流 `id:` 为事件哈希，subscribe 流 `id:` 为数字 seq——R 组断言使用后者。

### 11.3 发现的框架语义缺陷（e2e 直接产出）

| # | 缺陷 | 影响 | 关联用例 |
|---|------|------|---------|
| D1 | **channel 路径 HITL approve 恢复执行时工具参数丢失**：挂起时 pendingConfirm.input 正确，批准后恢复执行却收到空参数（报 content:null 校验错误） | H2/H5 以 test.fixme 标记；R5 断言放宽为"恰好一次终态" | H2/H5/R5 |
| D2 | **客户端断连后剩余 TEXT delta 不再持久化到事件流**：turn 继续完成（AGENT_END 可观测），但断连点之后的逐 delta 增量丢失，跨副本"逐 delta 续传"不可用 | R2 改为锁定尾部事件/seq 完整性 | R2 |
| D3 | **非沙箱模式 write_file→present_file 工作区镜像断裂**：write_file 落 SDK 本地会话目录，present_file 读 KV（经 syncWriteFileToKv 桥接，但桥接在回放链路未生效），file_ready 不产出 | F5/F10/U11/X9 以 test.fixme 标记 | F5/F10/U11/X9 |
| D4 | **ASKING 态下新 turn 被拒**：error 帧声明需先审批，与 api-thread-spec"挂起期间可发新消息"描述相反 | H6 按真实行为断言 | H6 |
| D5 | **hang 型停顿在模型流停滞前不产出任何事件**（AGENT_START 亦不落盘）：kill 副本后 B 侧无从判定 interrupted | R4 改用长流 slow（时长 > 租约 TTL）触发 | R4 |
| D6 | **ui.app_only 与 permissions.tools.ask 不能同 server 混配**（app_only 触发 registerReadOnly 路径短路 ask），且跨 server 同名裸名工具注册相互覆盖 | e2e 用 4 个逻辑 server 拆分语义（approval/bench/denied/cards）+ ActiveMCP.json 子集过滤 | 夹具设计 |

### 11.4 运维要点

- 录制器与 jar 必须**成对重启**（半开连接池会让录制流中断/丢 chunk）；录制脚本已内置 per-request 记录、空流丢弃、mtime 夹具缓存失效。
- mock LLM 缺夹具即 500（严格模式）；`MOCK_LLM_ALLOW_SYNTH=1` 仅限本地排障。
- 沙箱 mock 命令白名单：echo/ls/cat/mkdir/tar/base64/rm/printf/test/wc/stat/find/sort/head/tail/grep/sed/sh/true/false；白名单外命令记 /stats 并拒绝。
