...
public class A2AServerConfig {
    ...
    @Bean
    @DependsOn("reactAgent")
    public AgentScopeA2aServer a2aServer(ReActAgent reactAgent, OafConfig oafConfig) {
        var builder = ReActAgent.Builder.fromAgent(reactAgent);
        ...
        var server = AgentScopeA2aServer.builder(builder)
            .agentCard(card)
            .withTransport(transportProps)
            .build();

        log.info("A2A server configured with JSON-RPC transport");
        return server;
    }
}
